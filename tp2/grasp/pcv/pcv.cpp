...
vector<float> get_distances_from_cities_in_lrc(int city,
...
vector<int> get_cities_lesser_than_limit(vector<float> distances,
                                         vector<int> lrc, float limit) {
...
solution random_greedy_solution(int num_cities, float **distance_matrix,
                                float alpha) {
    ...
    while (sol.size() < num_cities) {
        ...
        std::vector<float> distances =
            get_distances_from_cities_in_lrc(last_city, distance_matrix, lrc);
        ...
        std::vector<int> cities =
            get_cities_lesser_than_limit(distances, lrc, limit);
        int end_interval = cities.size() - 1;

        int next_city;
        if (end_interval >= 0) {
            next_city =
                cities[get_random_number_from_interval(0, end_interval)];
        }
